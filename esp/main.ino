...
void i2s_adc(void *arg) {
  ...
  displayMessage("Recording Stopped", 0, 2);
  listLittleFS();
...
void connectToWiFi() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    wifiManager.resetSettings(); // Optional: Resets the saved credentials
...
void WiFiConnectTask(void *parameter) {
    connectToWiFiTask = true;
    connectToWiFi();
    wifiConnected = true;
    connectToWiFiTask = false;
    state = INITIAL_STATE;
    vTaskDelete(NULL);  // Delete the task once WiFi is connected
}
...
void playRecording(const char* filename) {
    ...
    while ((bytesRead = playbackFile.read(dataBuffer, i2s_read_len)) > 0) {
        ...
        size_t bytesWritten;
        esp_err_t err = i2s_write(I2S_PORT, dataBuffer, bytesRead, &bytesWritten, pdMS_TO_TICKS(1000));
        if (err != ESP_OK) {
...
void saveDefaultPassword(const String& password) {
  File file = LittleFS.open("/default_password.txt", FILE_WRITE);
  if (!file) {
  ...
  file.close();
  Serial.println("Default password saved.");
}

String readDefaultPassword() {
  File file = LittleFS.open("/default_password.txt", FILE_READ);
  if (!file) {
  ...
  file.close();
  Serial.println("Default password read: " + password);
  return password;
}
...
void checkForPasswordUpdate() {
    ...
    if (newPassword != defult_password) {
        ...
        saveDefaultPassword(defult_password);
    }
}
...
void setup() {
  ...
  LittleFSInit();
  listLittleFS();
  defult_password = readDefaultPassword();
  i2sInit();
  ...
  xTaskCreate(
        ...
        "WiFi Connect Task", // Name of the task
        4096,              // Stack size
        NULL,              // Parameter
        1,                 // Priority
...
bool check_password() {
  ...
  /*
    Serial.println("defult_password and inputSequence ");
    Serial.println(defult_password);
    Serial.println(inputSequence);
    Serial.print("Length of defult_password: ");
    Serial.println(defult_password.length());
    Serial.print("Length of inputSequence: ");
    Serial.println(inputSequence.length());
    */
    if (trimString(inputSequence) == trimString(defult_password))
    {
...
void loop() {
    ...
    if ( connectToWiFiTask == false && WiFi.status() != WL_CONNECTED && (millis() - lastCheck >= checkInterval)) {
    ...
    if (WiFi.status() == WL_CONNECTED && millis() - lastPasswordCheck >= passwordCheckInterval) {
